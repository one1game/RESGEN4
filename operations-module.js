...
export const operationsModule = {
  ...
  resolveDue() {
    ...
    this.state.factionStanding[faction] += incident ? -1 : 2;
  ...
  runConvoy() { if (this.state.convoy) return; const sector = this.state.control.sector || 'mining'; this.state.convoy = { sector, arrivesAt: Date.now() + 45000, cargo: 45 }; this.persist(); this.render(); window.showNotif?.('🚚 NPC-конвой отправлен в сектор'); },
  ...
  render() {
    ...
    const convoy = this.state.convoy ? `<span class="ops-codex-item">🚚 ${SECTORS[this.state.convoy.sector]?.label || this.state.convoy.sector} · ${fmtDuration(this.state.convoy.arrivesAt - Date.now())}</span>` : '<button class="ops-mini-btn" data-op-convoy>NPC CONVOY</button>';
