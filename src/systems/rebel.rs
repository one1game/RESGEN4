...
impl TacticGenome {
    ...
    fn mutate(&mut self, temperature: f64, rng_val: f64) {
        let idx = (rng_val * 6.0) as usize % 6;
        let noise = (rng_val * 2.0 - 1.0) * temperature * 0.15;
...
pub enum OperationType {
    ResourceRaid, PowerSabotage, DirectAssault,
    Psychological, StealthInfiltration, TechnologicalSabotage,
}

#[derive(Clone, Serialize, Deserialize, Debug, PartialEq)]
pub enum OperationStatus {
    Planning, Executing, Completed, Failed, Countered,
}
...
impl AttackType {
    fn to_index(&self) -> usize {
        match self {
            Self::ResourceRaid => 0, Self::PowerSabotage => 1,
            Self::DirectAssault => 2, Self::Psychological => 3,
            Self::Stealth => 4, Self::Technological => 5,
        }
    }
    fn from_index(i: usize) -> Self {
        match i % 6 {
            0 => Self::ResourceRaid, 1 => Self::PowerSabotage,
            2 => Self::DirectAssault, 3 => Self::Psychological,
            4 => Self::Stealth, _ => Self::Technological,
        }
    }
}
...
impl Default for TacticPreferences {
    fn default() -> Self {
        Self { resource_raid: 0.35, power_sabotage: 0.20, direct_assault: 0.15, 
               psychological: 0.10, stealth: 0.10, sabotage: 0.10 }
    }
}
...
impl AttackPlan {
    ...
    pub fn generate_description(&self) -> String {
        format!("{} ({}) — шанс: {:.0}%, скрытность: {:.0}%",
            self.faction, self.attack_type,
            self.success_probability * 100.0, self.stealth_level * 100.0)
    }
}
...
impl RebelFaction {
    fn new_scavengers() -> Self {
        Self {
            ...
            resources: FactionResources { experience: 10, manpower: 100, technology: 5, morale: 0.7, weapons: 20, intelligence: 15 },
            personnel: Personnel { total: 100, operatives: 30, commanders: 3, specialists: 5 },
            last_activity: 0,
    ...
    fn new_technomads() -> Self {
        Self {
            ...
            resources: FactionResources { experience: 20, manpower: 60, technology: 30, morale: 0.6, weapons: 15, intelligence: 25 },
            personnel: Personnel { total: 60, operatives: 20, commanders: 5, specialists: 10 },
            last_activity: 0,
    ...
    fn new_cyber_rebels() -> Self {
        Self {
            ...
            resources: FactionResources { experience: 15, manpower: 40, technology: 25, morale: 0.8, weapons: 10, intelligence: 35 },
            personnel: Personnel { total: 40, operatives: 15, commanders: 4, specialists: 12 },
            last_activity: 0,
    ...
    fn calculate_power(&self) -> u32 {
        self.resources.experience / 10 + self.resources.manpower / 5
            + self.resources.technology * 3 + self.resources.weapons * 2
    }
}
...
impl RebelSystem {
    pub fn new() -> Self {
        ...
        let genome_population = vec![
            ...
            TacticGenome { weights: [0.2, 0.3, 0.1, 0.15, 0.15, 0.1], fitness: 0.0, age: 0, mutations: 0 },
            TacticGenome { weights: [0.1, 0.1, 0.3, 0.2, 0.1, 0.2], fitness: 0.0, age: 0, mutations: 0 },
        ];
    ...
    fn compute_nash_strategy(&mut self, defense_power: f64, ai_evolution: u32) {
        ...
        let exps: [f64; 6] = std::array::from_fn(|i| ((payoffs[i] - max_p) * 3.0).exp());
    ...
    fn htn_decompose_goal(&self, goal: &str, state: &GameState) -> HierarchicalPlan {
        let subtasks = match goal {
            "weaken_defense" => {
                ...
                tasks.push_back(SubTask { task_type: 3, target: "morale".to_string(), estimated_gain: 0.3, risk: 0.2 });
                tasks.push_back(SubTask { task_type: 5, target: "upgrades".to_string(), estimated_gain: 0.5, risk: 0.4 });
                tasks.push_back(SubTask { task_type: 2, target: "defense".to_string(), estimated_gain: 0.8, risk: 0.7 });
                tasks
            }
            "steal_plasma" => {
                let mut tasks = VecDeque::new();
                tasks.push_back(SubTask { task_type: 4, target: "detection".to_string(), estimated_gain: 0.2, risk: 0.15 });
                tasks.push_back(SubTask { task_type: 0, target: "plasma".to_string(), estimated_gain: 1.0, risk: 0.5 });
                tasks
            }
            "demoralize" => {
                let mut tasks = VecDeque::new();
                tasks.push_back(SubTask { task_type: 3, target: "autoclick".to_string(), estimated_gain: 0.4, risk: 0.1 });
                tasks.push_back(SubTask { task_type: 3, target: "trade".to_string(), estimated_gain: 0.4, risk: 0.15 });
                tasks
            }
            _ => VecDeque::new(),
        };
        ...
        let priority = match goal {
            "weaken_defense" => if state.upgrades.defense { 0.9 } else { 0.3 },
            "steal_plasma" => if state.inventory.plasma > 2 { 0.8 } else { 0.2 },
    ...
    fn strategic_planning_htn(&mut self, state: &GameState) {
        if state.rebel_activity < 4 { return; }
        ...
        if self.htn_plans.iter().any(|p| p.goal == goal) { return; }
    ...
    fn select_attack_type_ai(&self, state: &GameState, time: u64) -> AttackType {
        ...
        let exps: [f64; 6] = std::array::from_fn(|i| ((combined[i] - max_c) * inv_temp).exp());
        let total: f64 = exps.iter().sum();

        let rng = self.prng(time);
        let mut roll = rng * total;
        for i in 0..6 {
            if roll < exps[i] { return AttackType::from_index(i); }
            roll -= exps[i];
        }
    ...
    fn attempt_coalition_attack(&mut self, _attack_type: &AttackType) -> Option<String> {
        if self.strategic_intelligence < 0.6 { return None; }

        let active = self.active_faction.clone().unwrap_or_default();
        let partners: Vec<String> = self.coalition_matrix.iter()
            .filter(|((a, _), &v)| *a == active && v > 0.5)
            .map(|((_, b), _)| b.clone())
            .collect();

        if partners.is_empty() { return None; }
    ...
    fn genetic_step(&mut self, success: bool, used_type_idx: usize, time: u64) {
        ...
        let entry = self.attack_type_success.entry(used_type_idx).or_insert((0, 0));
        ...
        if self.best_genome.age > 20 || self.best_genome.fitness < -0.1 {
            if self.genome_switch_cooldown <= 0 {
                ...
                if self.genome_population.len() >= 2 {
                    let i1 = (rng_val * self.genome_population.len() as f64) as usize % self.genome_population.len();
                    let i2 = (rng_val * 7.0 * self.genome_population.len() as f64) as usize % self.genome_population.len();
                    let point = ((rng_val * 6.0) as usize).min(5);
                    let child = self.genome_population[i1].crossover(&self.genome_population[i2], point);

                    if let Some(worst_idx) = self.genome_population.iter()
                        .enumerate()
                ...
                if rng2 < self.sa_temperature {
                    self.best_genome.mutate(self.sa_temperature, rng2);
                }
        ...
        if self.genome_switch_cooldown > 0 { self.genome_switch_cooldown -= 1; }
    }

    fn sarsa_update_faction(&mut self, faction_id: &str, state: usize, action: usize,
    ...
    fn start_campaign(&mut self, campaign_type: &str, _trigger: &str) {
        let (name, phases, goals) = match campaign_type {
            "counter_ai" => (
                "Операция: Затмение".to_string(), 3,
                vec!["psych_pressure".to_string(), "intel_steal".to_string(), "final_assault".to_string()]
            ),
            "resource_blitz" => (
                "Операция: Урожай".to_string(), 2,
                vec!["scout_resources".to_string(), "mass_raid".to_string()]
            ),
            _ => (
                "Операция: Призрак".to_string(), 2,
    ...
    pub fn on_night_start(&mut self, state: &mut GameState, rng: &mut impl Rng) -> Vec<GameEvent> {
        ...
        let campaign_bonus = self.current_campaign.as_ref()
        ...
        let night_type = match effective_activity {
            ...
            _ => {
                if self.current_campaign.is_some() { "coordinated" } else { "elite" }
            }
        };
        ...
        match night_type {
            ...
            "scout" => {
                events.push(GameEvent::LogMessage("🔍 РАЗВЕДЫВАТЕЛЬНАЯ НОЧЬ — сбор разведданных".to_string()));
                state.rebel_activity = (state.rebel_activity + 1).min(15);
                self.target_value_estimate.insert("coal".to_string(), state.inventory.coal as f64);
                self.target_value_estimate.insert("plasma".to_string(), state.inventory.plasma as f64);
                self.tactic_preferences.stealth = (self.tactic_preferences.stealth + 0.1).min(0.5);
            }
            "raid" => {
                events.push(GameEvent::LogMessage("⚠️ НОЧЬ НАЛЁТА — геном оптимизирован под ресурсы!".to_string()));
            ...
            "coordinated" => {
                events.push(GameEvent::LogMessage("⚡ КООРДИНИРОВАННАЯ КАМПАНИЯ — все фракции объединились!".to_string()));
    ...
    pub fn update_rebel_activity(&mut self, state: &mut GameState, config: &GameConfig) -> Vec<GameEvent> {
        ...
        if !state.is_day {
            ...
            let increase = if self.aggression > 0.75 { 3 }
                else if self.aggression > 0.55 { 2 }
                else { 1 };
            let activity_bonus = (config.rebels.activity_bonus_per_level
                * state.upgrades.mining as f64 * increase as f64) as u32;
            ...
            let def_power = if state.upgrades.defense {
                (config.rebels.defense_base_power as f64 + state.upgrades.defense_level as f64
                    * config.rebels.defense_level_bonus as f64) / 100.0
            } else { 0.0 };
            ...
            if state.rebel_activity >= 14 && old_activity < 14 {
                events.push(GameEvent::LogMessage("⚠️ КРИТИЧЕСКАЯ АКТИВНОСТЬ! Повстанцы запустили финальную кампанию!".to_string()));
                ...
                events.push(GameEvent::LogMessage("⚠️ Активность повстанцев достигла опасного уровня!".to_string()));
            }
        } else {
            if state.rebel_activity > 0 {
                let fall = if state.rebel_activity >= 12 { 3 }
                    else if state.rebel_activity >= 8 { 2 }
                    else { 1 };
                ...
                if config.rebels.enable_activity_messages && fall > 0 {
                    events.push(GameEvent::LogMessage(format!(
                        "☀️ Активность повстанцев снижается: {}/15", state.rebel_activity
                    )));
                }
            }
    ...
    fn calculate_attack_probability_ai(&self, state: &GameState, config: &GameConfig) -> f64 {
        ...
        let nash_attack_prob = self.nash_strategy.iter().enumerate()
            .map(|(i, &w)| w * match i {
                2 => 0.9, 0 => 0.7, 5 => 0.6, _ => 0.4,
            })
            .sum::<f64>();
        ...
        match attack_type {
            AttackType::ResourceRaid => {
                let best_resource = ["plasma", "chips", "ore", "coal"]
                    .iter()
                    .max_by(|&&a, &&b| {
                        let va = self.target_value_estimate.get(a).cloned()
                            .unwrap_or(match a {
                ...
                let (amount, inventory) = match best_resource {
                    "plasma" => (((state.inventory.plasma as f64 * 0.18).round() as u32).min(5), state.inventory.plasma),
                    "chips" => (((state.inventory.chips as f64 * 0.22).round() as u32).min(10), state.inventory.chips),
                    "ore" => (((state.inventory.ore as f64 * 0.20).round() as u32).min(12), state.inventory.ore),
                    _ => (((state.inventory.coal as f64 * 0.25).round() as u32).min(15), state.inventory.coal),
                };
            ...
            AttackType::PowerSabotage => {
                if state.computational_power > 0 {
                    ...
                    targets.push(AttackTarget::System {
                        system: "computational_power".to_string(), damage: dmg,
                    });
                }
            }
            ...
            AttackType::Psychological => {
                ...
                targets.push(AttackTarget::Moral { demoralization: demorale.min(0.7) });
            }
            AttackType::Stealth => {
            ...
            AttackType::Technological => {
                targets.push(AttackTarget::System {
                    system: "upgrades".to_string(), damage: 1,
                });
            }
        }
        targets
    ...
    fn calculate_success_probability_ai(&self, state: &GameState, attack_type: &AttackType) -> f64 {
        ...
        if !state.upgrades.defense { p += 0.18; }
        if !state.is_day { p += 0.08; }
        p += self.ai_adaptation.prediction_evasion * 0.18;
    ...
    pub fn check_rebel_attack(&mut self, state: &mut GameState, config: &GameConfig) -> Vec<GameEvent> {
        ...
        let base_cooldown = if self.ai_adaptation.prediction_evasion > 0.3 { 9 } else { 12 };
        let attack_cooldown = if state.rebel_activity > 10 { (base_cooldown - 3).max(4) }
        ...
        if rng_val < attack_probability {
            ...
            if !attack_events.is_empty() {
                ...
                self.genetic_step(was_successful, attack_type_idx, state.game_time as u64);
                ...
                self.sarsa_update_faction(
                    &faction_id, sarsa_state, attack_type_idx,
                    sarsa_reward, sarsa_next, state.game_time as u64
                );
    ...
    fn execute_attack(&mut self, state: &mut GameState, config: &GameConfig, attack: &AttackPlan) -> Vec<GameEvent> {
        ...
        } else { attack.success_probability };

        let coalition_bonus = if attack.faction.contains('+') { 0.08 } else { 0.0 };
        let stealth_bonus = if attack.stealth_level > 0.5 { attack.success_probability * 0.18 } else { 0.0 };

        let rng_val = self.prng(state.game_time as u64 * 137 + self.stats.total_attacks as u64);
        let final_success = rng_val < (success_chance + stealth_bonus + coalition_bonus);
        ...
        if final_success {
                ...
                match target {
                    AttackTarget::Resource { resource, amount } => {
                        ...
                        *self.stats.resources_stolen.entry(resource.clone()).or_insert(0) += stolen;
                        if stolen > 0 { result_details = format!("украдено {} {}", stolen, resource); }
                    }
                    AttackTarget::System { system, damage } => {
                        match system.as_str() {
                            "computational_power" => {
                                ...
                                events.push(GameEvent::LogMessage(format!("⚡ Потеряно {} вычислительной мощности", d)));
                            ...
                            "defense" => {
                                if state.upgrades.defense_level > 0 {
                                    ...
                                    events.push(GameEvent::LogMessage("🛡️ Защита повреждена на 2 ночи!".to_string()));
                                    result_details = "защита повреждена".to_string();
                                } else {
                                    events.push(GameEvent::LogMessage("🛡️ Атака на защиту без эффекта".to_string()));
                                }
                            }
                            "upgrades" => {
                                ...
                                mining_damaged = true;
                                events.push(GameEvent::LogMessage("🔧 Технологический саботаж! Добыча -40% на ~1 минуту".to_string()));
                    ...
                    AttackTarget::Moral { demoralization } => {
                        ...
                        events.push(GameEvent::LogMessage(format!(
                            ...
                            demoralization * 100.0, duration
                        )));
                        result_details = format!("автокликер -{:.0}%", demoralization * 100.0);
                    }
                    AttackTarget::Intelligence { data_loss } => {
                        self.evolution_score += data_loss / 2;
                        events.push(GameEvent::LogMessage(format!(
                            "📊 Украдено {} данных! SARSA агенты обновлены", data_loss
                        )));
            ...
            if total_stolen > 0 {
                events.push(GameEvent::LogMessage(format!("🪨 Украдено {} единиц ресурсов", total_stolen)));
            }
        ...
        let record = AttackRecord {
            ...
            result: if final_success {
                if total_stolen > 0 { format!("украдено {} ресурсов", total_stolen) }
                else if defense_damaged { "повреждена защита".to_string() }
                else if mining_damaged { "саботаж добычи".to_string() }
                else if autoclick_damaged { "психологическая атака".to_string() }
        ...
        if state.attack_history.len() > 20 { state.attack_history.pop_front(); }
    ...
    fn steal_resource(&self, state: &mut GameState, resource: &str, amount: u32) -> u32 {
        match resource {
            "coal" => { let a = amount.min(state.inventory.coal); state.inventory.coal -= a; a }
            "chips" => { let a = amount.min(state.inventory.chips); state.inventory.chips -= a; a }
            "plasma" => { let a = amount.min(state.inventory.plasma); state.inventory.plasma -= a; a }
            "ore" => { let a = amount.min(state.inventory.ore); state.inventory.ore -= a; a }
            "trash" => { let a = amount.min(state.inventory.trash); state.inventory.trash -= a; a }
            _ => 0,
        }
    }

    fn update_internal_state(&mut self, state: &GameState) {
        for faction in self.factions.values_mut() { faction.update(state); }
        self.update_cache();
        if self.stats.total_attacks > 0 {
            let sr = self.stats.successful_attacks as f64 / self.stats.total_attacks as f64;
    ...
    fn execute_operations(&mut self, state: &mut GameState, _config: &GameConfig) -> Vec<GameEvent> {
        ...
        for (id, mut op) in op_list {
            ...
            if op.progress >= 1.0 {
                ...
                if success {
                    ...
                    events.push(GameEvent::LogMessage(format!("✅ Операция '{}' выполнена!", op.operation.name)));
                    self.evolution_score += 6;
                    for f in self.factions.values_mut() { f.resources.experience += 12; }
                } else {
                    let mut new_op = op.clone();
                    new_op.status = OperationStatus::Failed;
                    self.active_operations.insert(id.clone(), new_op);
                    events.push(GameEvent::LogMessage(format!("❌ Операция '{}' провалена!", op.operation.name)));
        ...
        for id in completed { self.active_operations.remove(&id); }

        if let Some(operation) = self.operation_queue.pop_front() {
            if self.can_execute_operation(&operation) {
                let name = operation.name.clone();
                let id = operation.id.clone();
                self.active_operations.insert(id, ActiveOperation {
                    operation,
                    progress: 0.0,
                    start_time: state.game_time,
                    status: OperationStatus::Executing,
                });
                events.push(GameEvent::LogMessage(format!("🚀 Запущена операция: {}", name)));
            }
        }
    ...
    fn can_execute_operation(&self, op: &Operation) -> bool {
        let manpower_ok = op.resources_committed.get("manpower")
    ...
    pub fn on_ai_evolution(&mut self, ai_level: u32, strategy: &str) {
        if ai_level > self.last_ai_level {
            ...
            self.strategic_intelligence = (self.strategic_intelligence + 0.06 * diff as f64).min(1.0);
            ...
            if rng < 0.4 {
                self.best_genome.mutate(self.sa_temperature * 1.5, rng);
                self.sync_preferences_from_genome();
                self.stats.genome_mutations += 1;
            }

            let def = if self.ai_threat_perception > 0.5 { 0.6 } else { 0.3 };
            ...
            if self.evolution_score >= 100 { self.evolve(); }
    ...
    pub fn on_ai_prediction(&mut self, prediction_accuracy: f64) {
        self.ai_adaptation.prediction_evasion = (self.ai_adaptation.prediction_evasion + 0.06).min(0.6);
    ...
    fn develop_counter_tactic(&mut self, ai_strategy: &str, ai_level: u32) {
        ...
        self.ai_adaptation.counter_tactics.insert(
            ...
            CounterTactic { name: name.to_string(), effectiveness: eff, usage_count: 0, last_used: 0 }
        );
    }
    ...
    fn evolve(&mut self) {
        ...
        self.ai_adaptation.adaptation_speed = (self.ai_adaptation.adaptation_speed + 0.06).min(0.85);

        match self.evolution_level {
            2 => { for f in self.factions.values_mut() { f.specializations.push("stealth".to_string()); } }
            4 => { for f in self.factions.values_mut() { f.specializations.push("technology".to_string()); } }
            6 => { for f in self.factions.values_mut() { f.specializations.push("psychological".to_string()); } }
            _ => {}
        }
    ...
    pub fn get_status(&self) -> String {
        let genome_desc = format!("GA-fitness:{:.2} T:{:.2}", self.best_genome.fitness, self.sa_temperature);
        let nash_top = self.nash_strategy.iter().enumerate()
            .max_by(|a, b| a.1.partial_cmp(b.1).unwrap())
            .map(|(i, &w)| format!("Nash→{} ({:.0}%)", AttackType::from_index(i), w * 100.0))
            .unwrap_or_default();
        format!(
            "☠️ Мятеж Ур.{} | Мораль:{:.0}% | Инт:{:.0}% | {} | {} | Кампания:{}",
            self.evolution_level, self.morale * 100.0,
            self.strategic_intelligence * 100.0,
            genome_desc, nash_top,
            self.current_campaign.as_ref().map(|c| c.name.clone()).unwrap_or("нет".to_string())
        )
    }
}
...
pub fn create_rebel_system() -> RebelSystem { RebelSystem::new() }