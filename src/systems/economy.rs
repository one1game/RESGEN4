...
impl EconomySystem {
    ...
    pub fn buy_resource(
        ...
        if state.trade_blocked {
            events.push(GameEvent::LogMessage(
                "🔴 Торговля заблокирована (осада)!".to_string(),
            ));
            return events;
        }
        ...
        if amount == 0 || amount > 1000 {
            events.push(GameEvent::LogMessage(
                "❌ Некорректное количество".to_string(),
            ));
            return events;
        }
        ...
        let (cost_resource, cost_per_unit, gain_resource, gain_per_unit) = match resource {
            ...
            _ => {
                events.push(GameEvent::LogMessage(format!(
                    "❌ Неизвестный ресурс для покупки: {}",
                    resource
                )));
                return events;
            }
        ...
        if current < total_cost {
            events.push(GameEvent::LogMessage(format!(
                "❌ Недостаточно {} для покупки {} (нужно {}, есть {})",
        ...
        events.push(GameEvent::LogMessage(format!(
            "💱 Куплено: +{} {} за {} {}",
            gained, gain_resource, total_cost, cost_resource
        )));
        events
    }
    ...
    pub fn sell_resource(
        ...
        if state.trade_blocked {
            events.push(GameEvent::LogMessage(
                "🔴 Торговля заблокирована!".to_string(),
            ));
            return events;
        }
