// ======== src/game/state.rs (ИСПРАВЛЕНАЯ ВЕРСИЯ - С ЗАЩИТОЙ МОЩНОСТИ) ========
...
impl GameState {
    ...
    // ========== НОВЫЕ МЕТОДЫ ДЛЯ БЕЗОПАСНОЙ РАБОТЫ С МОЩНОСТЬЮ ==========
    
    /// Безопасно добавляет мощность с учётом максимального значения
    pub fn add_power_safe(&mut self, amount: u32) -> u32 {
        let old = self.computational_power;
        self.computational_power = (self.computational_power + amount).min(self.max_computational_power);
        let added = self.computational_power - old;
        
        // Обновляем тир мощности при необходимости
        let new_tier = (self.computational_power / 1000).min(20);
        if new_tier > self.power_tier {
            self.power_tier = new_tier;
        }
        
        added
    }
    
    /// Безопасно вычитает мощность (не уходит в负数)
    pub fn subtract_power_safe(&mut self, amount: u32) -> u32 {
        let old = self.computational_power;
        self.computational_power = self.computational_power.saturating_sub(amount);
        old - self.computational_power
    }
    
    /// Принудительно устанавливает мощность с проверкой границ
    pub fn set_power_safe(&mut self, power: u32) {
        self.computational_power = power.min(self.max_computational_power);
    }
    
    /// Получить мощность (гарантированно не больше максимума)
    pub fn get_power_clamped(&self) -> u32 {
        self.computational_power.min(self.max_computational_power)
    }
    
    /// Проверяет и исправляет мощность, если она превышает максимум
    pub fn clamp_power(&mut self) -> bool {
        if self.computational_power > self.max_computational_power {
