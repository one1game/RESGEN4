...
let _activeVoices = 0;
const MAX_VOICES = 8;
let _volumeMultiplier = parseFloat(localStorage.getItem('corebox_sound_volume') || '1.0');

function acquireVoice() {
    if (_activeVoices >= MAX_VOICES) return false;
    _activeVoices++;
    return true;
}

function releaseVoice(delayMs = 150) {
    setTimeout(() => {
        _activeVoices = Math.max(0, _activeVoices - 1);
    }, delayMs);
}

export function setVolumeMultiplier(value) {
    _volumeMultiplier = Math.max(0, Math.min(1, value));
    localStorage.setItem('corebox_sound_volume', _volumeMultiplier.toString());
}
...
async function getAudioCtx() {
    ...
    if (!audioCtx) {
        ...
        if (isMobile) {
            _volumeMultiplier *= 0.7;
        }
        if (navigator.hardwareConcurrency && navigator.hardwareConcurrency <= 2) {
            _volumeMultiplier *= 0.8;
        }
    }
...
function getMasterVolume() {
    return MASTER_VOLUME * _volumeMultiplier;
}
...
async function playModernTone({ frequency = 440, type = 'sine', duration = 0.15, volume = 0.3, attack = 0.02, decay = 0.1, sustain = 0.3, freqEnd = null, filterFreq = 2000, filterQ = 1, detune = 0 } = {}) {
    ...
    try {
        ...
        releaseVoice((duration || 0.15) * 1000);
    }
}
...
async function playDigitalClick(volume = 0.12) {
    ...
    try {
        ...
        gain.gain.setValueAtTime(volume * getMasterVolume(), ctx.currentTime);
...
async function playSuccessChime() {
    ...
    try {
        ...
        notes.forEach((f, i) => {
            ...
            const noteStart = start + i * 0.08;
            gain.gain.setValueAtTime(0, noteStart);
            gain.gain.linearRampToValueAtTime(0.08 * getMasterVolume(), noteStart + 0.01);
...
export const Sounds = {
    ...
    async shipSend() {
        ...
        try {
            ...
            gain.gain.setValueAtTime(0.12 * getMasterVolume(), ctx.currentTime);
    ...
    async nightStart() {
        await this.resume();
        await playSpaceNoise({ duration: 1.2, volume: 0.08, filterFreq: 60 });
    ...
    async toggleMute() {
        ...
        this.setMusicEnabled(isAudioEnabled);

        try {
            localStorage.setItem('corebox_sound_muted', (!isAudioEnabled).toString());
        } catch(e) {}
    ...
    setMusicEnabled(enabled) {
        try {
            ...
            console.warn('setMusicEnabled error:', e);
        }
    },
    ...
    getSoundStatus() {
        return {
            ...
            context: audioCtx?.state || 'suspended',
