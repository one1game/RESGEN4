...
export const Sounds = {
    ...
    async upgrade() {
        await this.resume();
        [523.25, 659.25, 783.99, 1046.50].forEach((f, i) => { setTimeout(() => { playModernTone({ frequency: f, type: 'sine', duration: 0.5, volume: 0.1, attack: 0.04 }); }, i * 50); });
    },
    async error() {
        await this.resume();
    ...
    async warning() {
        await this.resume();
        for (let i = 0; i < 3; i++) { setTimeout(() => { playModernTone({ frequency: 880, type: 'triangle', duration: 0.15, volume: 0.15, filterFreq: 2000 }); playSpaceNoise({ duration: 0.1, volume: 0.1, filterFreq: 1000, type: 'white' }); }, i * 400); }
    },
    async trade() {
        await this.resume();
    ...
    async toggleMute() {
        ...
        else if (isAudioEnabled && audioCtx) { await audioCtx.resume(); playModernTone({ frequency: 880, type: 'sine', duration: 0.1, volume: 0.1 }); }
        return isAudioEnabled;
    },
    isMuted() { return !isAudioEnabled; },
    getSoundStatus() { return { enabled: isAudioEnabled, context: audioCtx?.state || 'suspended' }; }
};
